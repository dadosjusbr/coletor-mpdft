...
def links_remuneration(month, year):
    links_type = {}
    link = f"{base_url}remuneracao&format=ods&iMes={month}&exercicio={year}"
...
def links_perks(month, year):
    links_type = {}
    link = f"{base_url}indenizacoes&format=ods&iMes={month}&exercicio={year}"
...
def links_temporary_funds(month, year):
    links_type = {}
    link = f"{base_url}verbastemporarias&format=ods&iMes={month}&exercicio={year}"
...
def download(url, file_path):
    try:
        ...
        sys.stderr.write(f"Não foi possível fazer o download do arquivo: {file_path}. O seguinte erro foi gerado: {excep}")
        os._exit(1)
...
def crawl(year, month, output_path):
    ...
    for element in urls_perks:
        pathlib.Path(output_path).mkdir(exist_ok=True)
        file_name_indemnity = element + "-" + "verbas-indenizatorias" + "-" + month + "-" + year + ".ods"
    ...
    for element in urls_temporary_funds:
        pathlib.Path(output_path).mkdir(exist_ok=True)
        file_name_indemnity = element + "-" + "verbas-temporarias" + "-" + month + "-" + year + ".ods"
