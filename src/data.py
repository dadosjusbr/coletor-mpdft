...
def load(file_names, year, month, output_path):
    ...
    temporarias = _read([i for i in file_names if "temporarias" in i][0])

    return Data(contracheque, indenizatorias, temporarias, year, month, output_path)


class Data:
    def __init__(self, contracheque, indenizatorias, temporarias, year, month, output_path):
        self.year = year
        self.month = month
        self.output_path = output_path
        self.contracheque = contracheque
        self.indenizatorias = indenizatorias
        self.temporarias = temporarias
    ...
    def validate(self):
        ...
        if not (
            ...
            or os.path.isfile(
                f"{self.output_path}/membros-ativos-verbas-temporarias-{self.month}-{self.year}.ods"
            )
        ):
